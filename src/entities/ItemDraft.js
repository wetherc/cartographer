...
/**
 * Turning the item form's raw control values into an inventory item. Split out
 * of `ui/ItemForm.js` for the same reason as `SpellDraft.js`: which fields
 * survive the chosen type is a rule about items, not about the form, and it is
 * what decides whether a library template round-trips intact.
 */
...
/**
 * The whole submitted item form. Fields the chosen type does not use are still
 * present, since the controls holding them still exist; `assembleItem` drops
 * them.
 ...
 * @property {string} notes carried through from the item being edited
...
/**
 * The item a submitted form describes, minus its id. Null rejects the submission
 * the way an empty name does: a stack of zero or fewer is not an item.
 *
 * Each optional block is gated on the type that uses it, so switching type
 * before submitting cannot leave armour fields on a rope. A zero AC bonus, an
 * empty buff, and an empty status list are all left out rather than stored as
 * zeroes, which keeps a saved item the same shape as a fresh one.
...
/**
 * One preset option's label. The parenthetical says whatever distinguishes that
 * kind of preset: its damage die for a weapon, its AC and weight for armour,
 * nothing for anything else, since a list of rope variants reads better without
 * empty brackets.
