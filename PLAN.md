...
- [ ] **Interior-node semantics** — the 13 castle-interior tiles are paintable brushes, but nothing in the data model distinguishes a "building interior" node from a region/subregion: the `MapNode` hierarchy is world -> region -> subregion -> POI only, so an interior floor plan is just another grid with no concept of "inside a structure" (no entry-from-door behavior, no interior-appropriate fog/entry defaults, no palette filtering to interior pieces when editing one). Needs a design decision — likely a `kind` field on `MapNode` — before interiors are more than loose tiles
