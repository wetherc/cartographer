...
test('encountersAt keeps encounters in the party node plus unbound ones', () => {
  ...
  assert.deepEqual(
    encountersAt(all, null).map((e) => e.id),
    ['e3'],
  );
});
...
test('encountersOnTile matches the exact tile, skips unbound, defeated, and null position', () => {
  const here = createEncounter('e1', 'Goblin', 7, {}, { nodeId: 'region', tileId: '1,1' });
  const sameNodeOtherTile = createEncounter(
    'e2',
    'Ogre',
    20,
    {},
    { nodeId: 'region', tileId: '2,2' },
  );
...
test('editEncounter rewrites blueprint fields but keeps live state', () => {
  const base = applyDamage(
    {
      ...createEncounter('e1', 'Goblin', 10, { AC: 13 }, { nodeId: 'world', tileId: '1,1' }),
  ...
  assert.deepEqual(
    edited.statBlock,
    fullBlock({ AC: 13 }),
    'hand-tuned stat block is not re-stamped',
  );
...
test('effectiveStatBlock layers active modifiers over the base block', () => {
  ...
  assert.equal(
    effective.AC,
    12 + (goblin.armor?.acBonus ?? 0),
    'armor bonus layers with modifiers',
  );
...
test('encountersNear keeps unbound encounters plus bound ones within the radius', () => {
  ...
  assert.deepEqual(
    encountersNear(all, null, 8).map((e) => e.id),
    ['e4'],
  );
});
...
test('editEncounter resets the noticed flag only when the location changes', () => {
  const noticed = {
    ...createEncounter('e1', 'Goblin', 10, {}, { nodeId: 'world', tileId: '1,1' }),
    noticed: true,
  };
...
test('withDefaults backfills gear on older saves without touching existing gear', () => {
  ...
  const armed = withDefaults(
    createEncounter('e2', 'Chief', 20, {}, null, {
      weapon: {
        name: 'Club',
        handling: 'melee',
        damage: [{ count: 1, sides: 4, damageType: 'bludgeoning' }],
