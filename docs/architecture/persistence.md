...
`nodes` array — the `TileGrid`'s node map flattened — plus `party`,
...
with no point of interest — and the undo history of the day multiplied
...
those fields from absence — and which every load already ran, so nothing
states a default twice. `deserialize` runs `withNodeDefaults` at the seam
rather than leaving the unpack to `toTileGrid`.
...
The reason a static table would be wrong: `Encounter.withDefaults` resolves
`weapon` and `armor` from the encounter's own level and tier. A table of
per-type defaults would omit a level-7 boss's weapon because it matches what a
level-1 mob would be given — and loading would hand the boss different gear.
...
GM-supplied images arrive as inline `data:` URLs. Stored that way, one
imported tile painted across a 30x30 region costs its whole base64 payload
once per cell — 18.5 MB of save for a 20 KB image. `storage/Assets.js` fixes
this: `hoistAssets` replaces every inline `data:` URL with an `asset:<key>`
...
stacked) and a handout's `image` — are listed in one traversal there, so
adding a third site is a single line.
...
costs the GM a handout picture instead of their map — and it keeps a history
snapshot from carrying a picture it did not change. The write order (payloads
first) limits the failure to the survivable shape: a campaign referencing a
payload the sidecar lacks renders the placeholder the renderer already draws,
while the reverse order could persist structure that references nothing. It
also settles the cross-tab case, since `isExternalSaveEvent` fires on the
campaign key and the payloads are already stored by then.
...
match) rather than by walking parsed state — after the tile codec below, a
tile's reference lives inside an encoded node's palette, which a state walk
cannot see without decoding first. The scan is skipped outright when there is
nothing to keep, which is every campaign that has never held an image.
...
Measured: the example campaign's save from 129,111 characters to 34,963, its
node list from 115,430 to 21,282, and a dense 40x40 region from 93,880 to
3,621 — the same region fully explored costs 15 more characters rather than
the 25,600 the per-cell form adds.
...
- Decoding degrades rather than throwing — an unreadable palette entry skips
  its cell, an unreadable run ends the stream — because Import persists what
  it reads before reloading, so a throw here is a save that cannot boot.
...
`storage/HistoryLog.js`. `saveCampaign` is the one save path: it writes the
campaign, then appends one delta produced by `storage/StateDiff.js`'s
`diffState` over the previous and new parsed states.
...
`campaign-builder:history:d<seq>` per delta — so a step is one small `setItem`
rather than a rewrite of the log. Measured on the example campaign, fifty
party steps cost 27,304 bytes of log where the previous ten-snapshot ring cost
...
the one property the old snapshot ring had for free. `HistoryLog` caches it,
stamped with the raw string it was parsed from, so the steady state costs a
string compare, and a tab that declined the cross-tab reload prompt cannot
diff against a save another tab replaced.
...
Both stores' file paths route through `storage/fileIO.js` — `downloadJSON` and
`readFileText`, the only two places the app touches `Blob`, object URLs, or
`FileReader`. New export/import features should call these rather than
re-rolling the browser plumbing; the planned Tauri desktop build swaps this
one file for native dialogs and the fs plugin.
