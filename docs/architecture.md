...
`CharacterTokens.js` (`src/party/CharacterTokens.js`) layers individual characters over that shared position: a `Character.location` of null means "with the party" (the token renders on the party's tile), while a non-null location is the character's own tile. `characterTokens(characters, partyPosition, nodeId)` resolves the named tokens to draw in a rendered node, `moveCharacter` relocates one character, and `recallAll` drops every individual location — the whole-party teleport. Movement permissions reuse `CharacterBinding.partyPermissions`: the GM moves the party (map clicks, which recall everyone) and any single character (the roster's place action); a bound player tab moves only its own character, whose steps reveal fog via the same `revealAround`.
