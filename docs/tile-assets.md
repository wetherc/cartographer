...
The joining contract: the waterline and the sand strand's landward edge cross tile edges at fixed offsets (28 and 36 units from the water edge), and the waterline is a *scalloped* wave — cusps every 16 units with each scallop bumping toward the water — so every piece meets every edge at a cusp. Because a cusp is orientation-agnostic (any two pieces meeting at one reproduce the interior motif), straights and corners join seamlessly in any arrangement, and only one outer corner and one inner corner are authored; the other three of each family are `<g transform>` mirrors of it. The water half's edges match plain water tiles.
