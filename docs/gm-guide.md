...
- **Load example** — replaces the current campaign with a complete demo
  campaign: a 32x32 overworld with a bay coastline and hand-shaped terrain,
  four outdoor subregions (two wilderness regions, the farming town of
  Briarwick, and the port of Saltmere) plus a dungeon interior and a castle
  keep, populated end to end as a playable arc. It ships an eleven-quest
  chain (goblin raids that trace back to the risen king in the barrow, with
  side threads through the port, the keep, the mine, and the wardstones), two
  staffed towns of NPCs, field enemies in every biome, minor bosses and a
  major boss, lore handouts, a bestiary of reusable mob templates, and a
  two-member party with kit and spell slots. Useful for seeing how a
  filled-in campaign looks before building your own. It also overwrites the
  current campaign, so it confirms first.
...
own after a few seconds. Every save in your GM tab, autosaves included, is
picked up by the other tabs, which is how you drive a player-facing screen from
your laptop without a server. A Play-mode tab takes the change without
...
held); the wheel zooms. The left button is reserved for painting in Build and
acting in Play.

**Undo stroke** (the Tools card, or Ctrl/Cmd+Z in Build mode) reverts the last
edit. A whole paint or erase drag counts as one edit, as does a region link or
a generation. This history is separate from the
...
nothing touches the node until you accept, so rerolling costs nothing. The
seed reproduces the layout, so note it down if you want to regenerate the same
map later. Accepting replaces the node's grid
...
level's stairs-up, and the bottom level has no stairs-down.
...
the six ability scores plus AC being the only stats an enemy carries (legends
always out-stat a level-matched mob), and arm the enemy with generic **gear**: a
...
**None**, for a non-bipedal beast, an ooze, or anything that carries no weapon
or wears no armor. That leaves the enemy unarmed (no attack button in combat)
or unarmored, with no default stamped back in. Both stay editable in
...
selectable; an empty one says so. Players see fewer: a nearby encounter enters
their sidebar only once it's been discovered, its tile
...
character) comes within detection range (twice the fog reveal radius). You see
a live encounter's marker slightly before its tile is revealed; anything
further stays hidden.

Every encounter row has an **edit** (pencil) action opening the same dialog the
add flow uses (name, HP, level/tier, and the map/tile placement), so you can
move an encounter without deleting and recreating it. Its live
...
chosen tile, defaulting to the selected one. Repeat foes are one click each.
The picker lists your campaign's saved templates alongside the
...
XP does not level a classed character on its own: each earned level waits as a
